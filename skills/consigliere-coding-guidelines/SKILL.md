---
name: consigliere-coding-guidelines
description: >-
  Agent-only reference for changing consigliere's shared, tracked material per AGENTS.md section 1.
  Use before editing any of that material, whether working as consigliere directly or as a soldier briefed on a consigliere-repo task.
  Covers the knowledge-placement decision tree, the one-owner rule for contracts, the inline-stub pattern for content moved into a skill, AGENTS.md size discipline, trigger hygiene for new skills, and repo style rules (one sentence per line, plain dash, no agent co-author, shellcheck-clean bin scripts, colocated tests, and verified-facts evidence).
user-invocable: false
---

# consigliere-coding-guidelines

Load this before changing consigliere's shared, tracked material, as defined by `AGENTS.md` section 1.
It exists because upstream firstmate's `AGENTS.md` grew from 585 to 958 lines between two restructures, entirely from conditional detail added inline instead of routed to its right home.
Applying the rules below on every change is what keeps that from happening here.
