...
2. Find the most recent real boss-authored message before the current `/rundown` invocation.
   System, developer, tool, watcher or monitoring, turn-end-guard (turn-end guard), away-supervisor (away-mode activation delivery), launch-brief (launch instruction), session-start (session-start nudge), and `from-consigliere` routing messages are operational input, not boss messages.
