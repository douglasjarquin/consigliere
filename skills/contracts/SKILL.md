...
1. The project is registered in `data/projects.md` (delivery mode + yolo) and its board is mapped in `data/boards.md`, beside the registry and keyed by the same project name (`<project> <owner> <number> [ready-label] [in-progress-label] [status-field]`; `docs/configuration.md`). If the board mapping is missing, tell the boss the one line they need to add and stop.
