...
5. For each issue you dispatch (up to the reported free capacity):
   ...
   b. Decide this issue's delivery mode and yolo posture per `AGENTS.md` section 6, defaulting to the project's standing registry posture unless the issue warrants otherwise.
...
6. Supervise every lane under section 7 (the foreground checkpoint). As a lane finishes and tears down, pull the next Ready/queued issue into it and repeat from step 5 until the column is empty or the boss says stop.
