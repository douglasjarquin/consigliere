...
1. Run `bin/cs-update.sh` and read its summary lines.
   It fast-forwards this repo's default branch from origin and sweeps every registered capo home; it never forces, merges, or stashes, and a tracked-files fast-forward never touches `data/`, `state/`, `config/`, `projects/`, or leftover `.no-mistakes/`, so in-flight work survives.
