...
# fetch_with_packed_refs_lock_guard and the CS_FLEET_SYNC_PACKED_REFS_LOCK_* knobs.
# Usage: cs-fleet-sync.sh [--all | <project-dir-or-name>]
# With no argument or with --all, every clone under this home's projects dir
# ($CS_HOME/projects, or $CS_PROJECTS_OVERRIDE) is swept.
# The single-project form accepts either a path (absolute, or relative to the
# caller's cwd) or a bare "<name>"/"projects/<name>" form, resolved against
# this home's projects dir. Bare names and "projects/<name>" forms prefer this
# home's projects dir before falling back to an explicit path. Example: from
# anywhere, `cs-fleet-sync.sh dotfiles-private` syncs just that one clone, same
# as passing its full projects/dotfiles-private path.
set -eu
...
FLEET_SYNC_PACKED_REFS_LOCK_RETRIES=${CS_FLEET_SYNC_PACKED_REFS_LOCK_RETRIES:-3}
FLEET_SYNC_PACKED_REFS_LOCK_RETRY_WAIT_SECS=${CS_FLEET_SYNC_PACKED_REFS_LOCK_RETRY_WAIT_SECS:-1}
FLEET_SYNC_PACKED_REFS_LOCK_AGE_SECS=${CS_FLEET_SYNC_PACKED_REFS_LOCK_AGE_SECS:-30}
...
usage() {
  echo "usage: cs-fleet-sync.sh [--all | <project-dir-or-name>]" >&2
}
...
# is provably stale per cs-lock-lib.sh (still present, mtime age past the
...
fetch_with_packed_refs_lock_guard() {
  ...
  if [ -n "$lock" ] && [ -e "$lock" ]; then
    if cs_lock_is_provably_stale "$lock" "$PROJ" "$FLEET_SYNC_PACKED_REFS_LOCK_AGE_SECS"; then
      if ! rm -f "$lock"; then
...
prune_gone_branches() {
  ...
  # net. Set CS_FLEET_PRUNE=0 to skip pruning entirely.
  [ "${CS_FLEET_PRUNE:-1}" != "0" ] || return 0
...
# --all, or no argument: sweep every clone under this home's projects dir.
[ -d "$PROJECTS" ] || exit 0
