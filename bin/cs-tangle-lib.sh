...
# Shared worktree-tangle guard for the consigliere-on-itself case.
# Usage: . bin/cs-tangle-lib.sh
#
# Consigliere is a treehouse-pooled git repo of itself: soldier worktrees and
# capo homes are all linked `git worktree`s of the same repo, while the
# PRIMARY checkout (the repo root consigliere operates from) is a normal checkout
# on a real branch - normally the default branch, main. The "worktree tangle"
# failure mode is a soldier spawned to work on consigliere ITSELF branching and
# committing in the primary checkout instead of its own disposable worktree,
# stranding the primary on a feature branch (e.g. cs/readme-restructure-d3).
#
# cs_primary_tangle_branch detects exactly that and nothing else: a NAMED,
...
# which is how every linked worktree and capo home legitimately sits on the
