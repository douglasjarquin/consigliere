...
# Precedence: CS_HARNESS_OVERRIDE (test/escape seam) -> config/host/harness.conf ->
...
cs_harness_detect_root() {
  ...
  file="$config_dir/host/harness.conf"
  if [ -f "$file" ]; then
...
# from config/host/permission-mode.conf, or empty when the file has no record for it.
...
cs_harness_permission_mode() {
  ...
  file="$(cs_harness_config_dir)/host/permission-mode.conf"
  [ -f "$file" ] || return 0
  while read -r key mode rest || [ -n "$key" ]; do
    case "$key" in ''|'#'*) continue ;; esac
    if [ -z "$mode" ] || [ -n "$rest" ]; then
      printf 'cs-harness: config/host/permission-mode.conf needs exactly "<harness> <mode>" per line: %s\n' "$key" >&2
      return 1
    fi
    case "$key" in
      claude) ;;
      codex)
        printf 'cs-harness: config/host/permission-mode.conf does not configure codex; its autonomy flag is fixed\n' >&2
        return 1
        ;;
      *)
        printf 'cs-harness: unknown harness "%s" in config/host/permission-mode.conf\n' "$key" >&2
        return 1
        ;;
    esac
    ...
    if [ "$key" = "$h" ]; then
      if [ -n "$found" ]; then
        printf 'cs-harness: duplicate config/host/permission-mode.conf record for %s\n' "$h" >&2
        return 1
      fi
      found=$mode
    fi
...
# `acceptEdits`) in config/host/permission-mode.conf; every other home keeps full autonomy.
