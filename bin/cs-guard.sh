...
# cs-wake-drain.sh after it empties queued wakes, and by session start in
# read-only advisory mode when another session holds the fleet lock.
# First, always warn if the consigliere primary checkout (CS_ROOT) is on a named
# non-default branch, because that means consigliere-on-itself work landed in the
...
# missing or older than CS_GUARD_GRACE seconds, prints a loud, clearly delimited
# banner so the agent cannot skim past it in the tool output of whatever it was
# doing - the one channel codex always has. The full banner is emitted once per
# distinct staleness episode in this CS_HOME (keyed to beacon mtime or absence);
...
queue_pending=false
READ_ONLY=${CS_GUARD_READ_ONLY:-0}
case "$READ_ONLY" in 1|true|TRUE|yes|YES) READ_ONLY=1 ;; *) READ_ONLY=0 ;; esac
CONTINUE_LINE=${CS_GUARD_CONTINUE_LINE:-This is a supervision warning only; the guarded operation WILL still run.}
...
cs_guard_clear_stale_banner() {
  rm -f "$STALE_BANNER_MARKER" 2>/dev/null || true
}

# Worktree-tangle alarm, checked FIRST and independent of in-flight tasks: the
# consigliere PRIMARY checkout (CS_ROOT) must stay on its default branch. If a
# soldier's branch/commits landed here instead of in its own isolated worktree,
...
# only: detached HEAD (linked worktrees, capo homes) never trips this.
tangle_branch=$(cs_primary_tangle_branch "$CS_ROOT" || true)
if [ -n "$tangle_branch" ]; then
  ...
  trule='━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━'
  {
    ...
    printf "●  %s is on '%s', not its default branch '%s'.\n" "$CS_ROOT" "$tangle_branch" "$tangle_default"
    printf '●  A soldier likely branched/committed in the primary instead of its own worktree.\n'
...
if [ "$in_flight" -eq 0 ]; then
  ...
  [ "$READ_ONLY" -eq 1 ] || cs_guard_clear_stale_banner
  exit 0
fi

[ -s "$CS_WAKE_QUEUE" ] && queue_pending=true
...
if [ "$watcher_fresh" = false ]; then
  episode_key=$(cs_guard_stale_episode_key "$STATE")
  episode_key=${episode_key%$'\n'}
  print_full_banner=0
  if [ "$READ_ONLY" -eq 1 ]; then
    cs_guard_stale_banner_seen "$STATE" "$episode_key" || print_full_banner=1
  elif cs_guard_claim_stale_banner "$STATE" "$episode_key"; then
    print_full_banner=1
  fi
  if [ "$print_full_banner" -eq 1 ]; then
    ...
    rule='━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━'
    {
      ...
      if [ "$READ_ONLY" -eq 1 ]; then
        ...
        printf '●  Trust the emitted supervision protocol; do not use shell & for watcher repair.\n'
      fi
      printf '●  %s\n' "$CONTINUE_LINE"
      printf '●  %s\n' "$fix"
      printf '●%s\n' "$rule"
...
if "$queue_pending"; then
  if [ "$READ_ONLY" -eq 1 ]; then
    ...
    echo "WARNING: queued wakes pending - drain them with bin/cs-wake-drain.sh before anything else." >&2
  fi
fi
exit 0
