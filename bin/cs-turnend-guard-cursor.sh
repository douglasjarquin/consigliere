#!/usr/bin/env bash
# Cursor `stop` hook adapter for a consigliere PRIMARY session: the park model.
#
...
# primary supervision:
#
#   PARK      while supervision is needed, foreground bin/cs-watch-arm.sh and
...
#   - CS_CURSOR_TURNEND_LOOP_CEILING bounds the payload's own loop_count from
#     inside, deliberately BELOW the registered loop_limit, so consigliere's bound
...
#
# SUPERSESSION. A boss message typed while this hook is parked is accepted
...
# cs_watch_arm_pi. Cursor identity keeps parking despite a leaked
...
LOOP_CEILING=${CS_CURSOR_TURNEND_LOOP_CEILING:-180}
BLOCK_BUDGET=${CS_CURSOR_TURNEND_BLOCK_BUDGET:-3}
ARM_ATTEMPTS=${CS_CURSOR_PARK_ATTEMPTS:-2}
POLL=${CS_CURSOR_PARK_POLL:-2}
LOCK_ATTEMPTS=${CS_CURSOR_LOCK_ATTEMPTS:-50}
...
# shellcheck source=bin/cs-session-lock-lib.sh
. "$SCRIPT_DIR/cs-session-lock-lib.sh"
...
lock_acquire_bounded() {  # <lock>
  ...
  while [ "$attempt" -lt "$LOCK_ATTEMPTS" ]; do
    cs_lock_try_acquire "$lock" && return 0
    attempt=$((attempt + 1))
...
emit_followup() {  # <kind> <body> [reset-budget]
  ...
  cs_operational_input_encode "$kind" "$body" encoded || exit 0
...
$reason"
  cs_operational_input_encode turn-end-guard "$body" encoded || exit 0
  response=$(jq -n --arg m "$encoded" '{followup_message:$m}' 2>/dev/null) || exit 0
...
current_session_still_ours() {
  ...
  cs_session_lock_owned_by_self "$STATE"
}
...
# case, delegated to bin/cs-lock.sh so acquisition keeps its single owner.
if ! cs_session_lock_owned_by_self "$STATE"; then
  LOCK_PID=$(cat "$STATE/.lock" 2>/dev/null || true)
  case "$LOCK_PID" in ''|*[!0-9]*) exit 0 ;; esac
  cs_harness_pid_alive "$LOCK_PID" && exit 0
  "$SCRIPT_DIR/cs-lock.sh" >/dev/null 2>&1 || exit 0
  cs_session_lock_owned_by_self "$STATE" || exit 0
fi
...
if [ "$LOOP_COUNT" -ge "$LOOP_CEILING" ]; then
  ...
  cs_supervision_needed "$STATE" "$GRACE" || exit 0
  emit_followup turn-end-guard "CONSIGLIERE SUPERVISION FOLLOW-UP CEILING REACHED - this session has taken $LOOP_COUNT consecutive hook-driven turns without a boss message, so automatic wake delivery stops here to bound the loop. Queued wakes stay durable: run bin/cs-wake-drain.sh, handle them, and run its exact WAKE_ACK_REQUIRED command. Supervision resumes automatically at the next turn end after the boss's next message."
fi

# Away mode owns the watcher and its own triage; never park and never wake.
[ -e "$STATE/.afk" ] && exit 0

if ! cs_supervision_needed "$STATE" "$GRACE"; then
  budget_reset_if_ours
  exit 0
fi
...
while [ "$attempt" -lt "$ARM_ATTEMPTS" ]; do
  ...
  if [ -n "$ARM_OUT" ]; then
    "$SCRIPT_DIR/cs-watch-arm.sh" >"$ARM_OUT" 2>&1 &
  else
    "$SCRIPT_DIR/cs-watch-arm.sh" >/dev/null 2>&1 &
  fi
  ARM_PID=$!
...
if [ "$ACTIONABLE" -eq 1 ]; then
  ...
  emit_followup watcher "consigliere watcher wake - one supervision event needs a handling turn now.
$WAKE

Run bin/cs-wake-drain.sh first, handle the wake, then run its exact WAKE_ACK_REQUIRED --ack-through command. Until that post-handling acknowledgement, interruption leaves the wake durable for idempotent re-handling. This stop hook owns watcher continuity: when the handling turn ends, the next needed cycle parks automatically - do NOT run bin/cs-watch-arm.sh after an ordinary wake." reset-budget
fi
...
# time: bin/cs-turnend-guard.sh owns the block decision and its banner for every
# harness, and --cursor tells it this is Cursor's own registration rather than
# the Claude-settings duplicate.
GUARD_ERR=$(mktemp "${TMPDIR:-/tmp}/cs-turnend-cursor.XXXXXX") || exit 0
printf '%s' "$PAYLOAD" | "$SCRIPT_DIR/cs-turnend-guard.sh" --cursor 2>"$GUARD_ERR"
GUARD_RC=$?
