#!/usr/bin/env bash
# Safe, home-scoped (re-)arm of the consigliere watcher, with honest verification.
#
# The watcher (bin/cs-watch.sh) blocks until it has an actionable wake to
...
# call and NOTIFIES on exit, so consigliere must run this script as the harness's
...
# (bin/cs-claude-stop-autoarm.sh), where the harness owns the process group and
...
# On a harness with a PreToolUse-equivalent hook, bin/cs-arm-pretool-check.sh
...
# liveness beacon (state/.last-watcher-beat) is fresh within CS_GUARD_GRACE (the
# single source of truth, shared with cs-watch.sh and fm-guard.sh), and prints
...
#
# --restart: stop ONLY this CS_HOME's watcher (the pid recorded in THIS home's
...
# bin/cs-watch.sh`: that pattern matches every consigliere home's watcher
...
CONFIRM_TIMEOUT=${CS_ARM_CONFIRM_TIMEOUT:-$ARM_CONFIRM_DEFAULT}
# Poll interval while attached to an existing healthy watcher.
ATTACH_POLL=${CS_ARM_ATTACH_POLL:-0.5}
CYCLE_LOG="$STATE/.watch-cycle-exits.log"
CYCLE_LOG_LOCK="$STATE/.watch-cycle-exits.lock"
CYCLE_LOG_MAX_BYTES=${CS_WATCH_CYCLE_LOG_MAX_BYTES:-262144}
CYCLE_LOG_KEEP_LINES=${CS_WATCH_CYCLE_LOG_KEEP_LINES:-1000}
ARM_PID=${BASHPID:-$$}
...
cycle_log_append() {
  ...
  [ "$cycle_active" -eq 1 ] || return 0
  ended_at=$(date +%s)
  beacon_age=$(cs_path_age "$BEAT")
  lock_after=$(lock_snapshot)

  i=0
  while ! cs_lock_try_acquire "$CYCLE_LOG_LOCK"; do
    [ "$i" -lt 20 ] || return 0
    sleep 0.02
    i=$((i + 1))
  done
  ...
  cs_lock_release "$CYCLE_LOG_LOCK"
  cycle_active=0
}
...
cycle_mark_predecessor_successor() {
  local successor=$1 predecessor=${CS_WATCH_PREDECESSOR_ARM_PID:-} i tmp
  case "$predecessor" in
    ''|*[!0-9]*) return 0 ;;
  esac
...
clear_stale_recorded_watcher_lock() {
  ...
  lock_home=$(cat "$WATCH_LOCK/cs-home" 2>/dev/null || true)
  lock_path=$(cat "$WATCH_LOCK/watcher-path" 2>/dev/null || true)
  lock_identity=$(cat "$WATCH_LOCK/pid-identity" 2>/dev/null || true)
  [ "$lock_home" = "$CS_HOME" ] || return 0
  [ "$lock_path" = "$WATCH" ] || return 0
  [ -n "$lock_identity" ] || return 0
  cs_lock_clean_known_files "$WATCH_LOCK"
...
healthy_watcher() {
  HEALTHY_PID=
  HEALTHY_IDENTITY=
  cs_watcher_healthy "$STATE" "$WATCH" "$GRACE" "$CS_HOME" || return 1
  HEALTHY_PID=$CS_WATCHER_HEALTHY_PID
  HEALTHY_IDENTITY=$CS_WATCHER_HEALTHY_IDENTITY
}
...
close_unobserved_cycle() {
  ...
  i=0
  while ! cs_lock_try_acquire "$WATCH_DELIVERY_LOCK"; do
    [ "$i" -lt 20 ] || {
      fail_unexplained_cycle
      return 1
    }
  ...
  cs_lock_release "$WATCH_DELIVERY_LOCK"
  if [ -n "$reason" ]; then
    printf '%s\n' "$reason"
    return 0
  fi
...
if [ "$mode" = handling-delivered ]; then
  cs_pid_alive "$handling_watcher_pid" \
    && cs_watcher_lock_matches_pid "$STATE" "$WATCH" "$handling_watcher_pid" "$CS_HOME"
  exit $?
fi
...
if [ "$mode" = restart ]; then
  ...
  if cs_pid_alive "$lock_pid"; then
    if cs_watcher_lock_matches_pid "$STATE" "$WATCH" "$lock_pid" "$CS_HOME"; then
      kill -TERM "$lock_pid" 2>/dev/null || true
      # Wait for it to actually exit before relaunching, so the fresh watcher
      # either takes a released lock or reclaims a now-dead-pid stale lock instead
      # of seeing the dying one as a live holder and no-opping.
      i=0
      while [ "$i" -lt 50 ] && cs_pid_alive "$lock_pid"; do
        sleep 0.1
        i=$((i + 1))
      done
    else
...
# wake exit propagates out so the harness re-notifies consigliere.
child=
child_out=
cleanup_child() {
  if [ -n "$child" ] && cs_pid_alive "$child"; then
...
if [ -n "${CS_WATCH_PREDECESSOR_ARM_PID:-}" ]; then
  CS_WATCH_HANDLING_SUCCESSOR=1 "$WATCH" >"$child_out" &
else
  "$WATCH" >"$child_out" &
fi
child=$!
cycle_begin "$child" started "$(cs_pid_identity "$child" 2>/dev/null || true)"
child_done=0
...
while :; do
  ...
  if [ "$child_done" -eq 0 ] && ! cs_pid_alive "$child"; then
    wait "$child"
    rc=$?
    child_done=1
    owned_child_finished "$rc"
