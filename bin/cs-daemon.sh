...
#     active alert (config/host/wedge-alarm.conf; default auto = macOS Notification
...
#     CS_WEDGE_ALARM_CHANNEL   override config/host/wedge-alarm.conf with one directive
...
# can reach the boss away from the machine. Config: config/host/wedge-alarm.conf (LOCAL,
...
wedge_alarm_configured_channels() {
  ...
  cfg="${CS_CONFIG_OVERRIDE:-$CS_HOME/config}/host/wedge-alarm.conf"
  if [ -f "$cfg" ]; then
...
wedge_alarm_notify() {  # <summary> <marker>
  ...
  for ch in "${channels[@]}"; do
    ...
    case "$ch" in
      '') log "wedge alarm: no OS-level alert channel on $(uname); durable marker $marker is the only signal - set config/host/wedge-alarm.conf (e.g. a command: directive)" ;;
