...
#
# SCOPE IS DELIBERATELY ASYMMETRIC (config/host/activation.conf):
...
mode=afk-only
if [ -f "$CONFIG/host/activation.conf" ]; then
  mode=$(awk 'NF && $0 !~ /^[[:space:]]*#/ { gsub(/[[:space:]]/, "", $0); print; exit }' "$CONFIG/host/activation.conf")
fi
case "$mode" in
  always|afk-only|off) ;;
  *) decision "refuse: config/host/activation.conf is '${mode:-<empty>}', expected always|afk-only|off"; exit 0 ;;
esac
