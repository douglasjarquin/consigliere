#!/usr/bin/env bash
# Run one bounded foreground watcher checkpoint. Codex cannot reason during a
# foreground tool call, so this is consigliere's ONLY supervision wait shape:
...
Run bin/cs-watch.sh in the foreground for a bounded checkpoint.
