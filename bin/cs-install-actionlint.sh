#!/usr/bin/env bash
# cs-install-actionlint.sh - install CI's pinned, verified actionlint build.
#
...
#
# Usage:
#   cs-install-actionlint.sh <destination-directory>
set -eu

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
VERSION="$("$ROOT/bin/cs-lint-workflows.sh" --required-version)"

die() {
  printf 'cs-install-actionlint.sh: %s\n' "$*" >&2
  exit 1
}

DESTINATION=${1:?usage: cs-install-actionlint.sh <destination-directory>}
...
TMP=$(mktemp -d "${RUNNER_TEMP:-${TMPDIR:-/tmp}}/cs-actionlint.XXXXXX")
trap 'rm -rf "$TMP"' EXIT
...
while ! curl -fsSL "$URL" -o "$TMP/$ARCHIVE"; do
  [ "$download_attempt" -lt "$DOWNLOAD_ATTEMPTS" ] || {
    printf 'cs-install-actionlint.sh: download failed after %s attempts\n' "$DOWNLOAD_ATTEMPTS" >&2
    exit 1
  }
  printf 'cs-install-actionlint.sh: download attempt %s failed; retrying\n' "$download_attempt" >&2
...
[ "$ACTUAL_SHA256" = "$SHA256" ] || {
  printf 'cs-install-actionlint.sh: checksum mismatch for %s (expected %s, got %s)\n' \
