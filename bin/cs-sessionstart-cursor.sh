...
# It is a thin transport around bin/cs-sessionstart-run.sh, which remains the
...
#
# Usage: cs-sessionstart-cursor.sh --source <source>
...
DIGEST=$("$SCRIPT_DIR/cs-sessionstart-run.sh" --source "$SOURCE" </dev/null 2>/dev/null || true)
[ -n "$DIGEST" ] || exit 0
