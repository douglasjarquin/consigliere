...
# failed or blocked would degrade herdr itself for every pane on the machine,
# while a dropped record costs only latency: the watcher's poll loop and its
# level reconcile remain the fail-closed backstop.
set -u
