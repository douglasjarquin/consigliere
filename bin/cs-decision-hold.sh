#!/usr/bin/env bash
# cs-decision-hold.sh - deterministic mechanics for durable boss decisions.
#
# The semantic policy is owned once by the decision-hold-lifecycle skill. This
# script never reads report, visual-review, chat, or terminal prose to guess
# whether a decision exists. The invoking agent inventories unresolved
# decisions, assigns stable keys, and routes dependent work. This script
# supplies deterministic identities, creates and verifies structured tasks-axi
# boss holds, records completion attestation in the originating task's
# metadata, and closes a hold only after a durable decision record has been
# linked to existing dependent work.
#
...
# All backlog mutations run in the active CS_HOME, which keeps main-home and
# capo-home ownership aligned with the work that discovered the decision.
#
# Usage:
#   cs-decision-hold.sh id <origin-id> <decision-key>
#   cs-decision-hold.sh hold <origin-id> <decision-key> \
#     --title <title> --reason <reason> [--repo <repo>]
#   cs-decision-hold.sh complete <origin-id> (--none | <decision-key>...)
...
# no unresolved boss decision. Later review passes may add keys; a live task's
...
# bin/cs-classify-lib.sh) are kept literally for format compatibility.
set -eu
...
BOSS_HELD_VERB=${CS_CLASSIFY_BOSS_HELD_VERB:-$CS_CLASSIFY_BOSS_HELD_VERB_DEFAULT}
...
tasks_axi() {
  (cd "$CS_HOME" && tasks-axi "$@")
}

require_tasks_axi() {
  cs_tasks_axi_compatible || fail "compatible tasks-axi is required"
...
verify_hold_active() {  # <hold-id>
  ...
  [ "$state" = queued ] || fail "boss hold $id is not queued (state=$state)"
  [ "$held" = yes ] || fail "boss hold $id is not active"
  [ "$kind" = captain ] || fail "backlog item $id is not kind captain"
  [ "$hold_kind" = captain ] || fail "backlog item $id is not held for the boss"
}
...
verify_hold_resolved() {  # <hold-id>
  ...
  case "$body" in
    *"Resolution recorded by cs-decision-hold."*"Routed work:"*) return 0 ;;
  esac
  return 1
}
...
verify_hold_durable() {  # <hold-id>
  ...
  fail "boss decision $id is neither actively held nor durably resolved"
}
...
verify_resolution_identity() {
  ...
  resolution_prefix='"Resolution recorded by cs-decision-hold.\nDecision digest: '
  case "$hold_body" in
    "$resolution_prefix"*) resolution_fields=${hold_body#"$resolution_prefix"} ;;
    *) fail "boss hold $id has no retry identity record" ;;
  esac
  ...
  [ "$recorded_digest" = "$decision_digest" ] \
    || fail "boss hold $id records a different boss decision"
  [ "$recorded_routes" = "$routed_csv" ] \
    || fail "boss hold $id records different routed work"
}
...
command_hold() {
  ...
  require_tasks_axi
  origin_exists_here "$origin" || fail "origin $origin is not owned by the active home $CS_HOME"
  id=$(hold_id "$origin" "$key")
  if show=$(task_show "$id"); then
    state=$(show_field "$show" state)
    kind=$(show_field "$show" kind)
    existing_title=$(show_field "$show" title)
    [ "$state" != "done" ] || fail "boss decision $id is already durably resolved; use a new decision key for a new decision"
    [ "$kind" = captain ] || fail "existing backlog identity $id is not kind captain"
    [ "$existing_title" = "$title" ] || fail "existing boss hold $id has a different title"
  else
    ...
    [ -n "$repo" ] || repo=consigliere
    validate_one_line repo "$repo"
    body=$(printf 'Origin: %s\nDecision key: %s\nState: awaiting boss decision.' "$origin" "$key")
    tasks_axi add "$id" "$title" --kind captain --repo "$repo" --body "$body" >/dev/null \
      || fail "could not create boss decision item $id"
  fi
  tasks_axi hold "$id" --reason "$reason" --kind captain >/dev/null \
    || fail "could not activate boss hold $id"
...
EOF
  ...
  while IFS=$'\t' read -r key _verb _summary; do
    ...
    list_has_key "$keys" "$key" \
      || fail "open structured decision $origin/$key has no boss-held inventory entry"
  done <<EOF
$open
EOF
  ...
  if [ "$has_meta" = 1 ]; then
    ...
    # live status fold does not duplicate the same Boss's Call item. The verb is
    # kept literally 'captain-held' (cs-classify-lib.sh) for format compatibility.
    while IFS=$'\t' read -r key _verb _summary; do
      [ -n "$key" ] || continue
      list_has_key "$keys" "$key" || continue
      printf '%s [key=%s]: tracked by %s\n' "$BOSS_HELD_VERB" "$key" "$(hold_id "$origin" "$key")" >> "$status_file"
      key_seen=1
    done <<EOF
$raw_open
EOF
...
command_resolve() {
  ...
  for dep in $routed; do
    ...
    case ",$blocked," in
      ...
      *)
        case "$hold_body" in
          *"Resolution recorded by cs-decision-hold."*"- $dep"*) : ;;
