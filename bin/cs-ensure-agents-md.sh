...
# This is a worktree utility for soldiers, not a supervision script, so it does
# not call cs-guard.sh.
# Usage: cs-ensure-agents-md.sh [repo-or-worktree-dir]
set -eu

usage() {
  echo "usage: cs-ensure-agents-md.sh [repo-or-worktree-dir]" >&2
}
...
# Refuse a case-variant real memory file. On a case-insensitive filesystem an
# existing lowercase agents.md satisfies every [ -e AGENTS.md ] test below, so
# the script would emit a CLAUDE.md symlink whose uppercase literal target
