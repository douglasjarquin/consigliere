...
`mise.toml`, `mise-tasks/dev/`, `docker/dev/Dockerfile`, `docker-compose.yml`, `.cursor/environment.json`, and `scripts/ci/` are a purely additive dev-tools suite (mise for tooling/tasks, `aube` where JS package-manager work needs it, a single container for local dev, CI, and Cursor Cloud Agents) - see [`docs/dev-environment.md`](docs/dev-environment.md).
