#!/usr/bin/env bash
# Behavior tests for cs-fleet-sync.sh drift handling.
#
# cs-fleet-sync fast-forwards a clone that is cleanly on its default branch. This
...
# (fetch_with_packed_refs_lock_guard, backed by bin/cs-lock-lib.sh's shared
...
TMP_ROOT=$(cs_test_tmproot cs-fleet-sync-tests)
HOME_N=0

# --- fixtures ---------------------------------------------------------------

# new_home: fresh isolated CS_HOME with an empty projects/ dir. Each test gets its
...
run_sync() {
  local home=$1
  shift
  CS_HOME="$home" CS_ROOT_OVERRIDE="$ROOT" "$ROOT/bin/cs-fleet-sync.sh" "$@" 2>/dev/null
}
...
# lsof shims mirror tests/cs-teardown.test.sh: no-holder (provably free), a live
...
# (CS_FLEET_SYNC_PACKED_REFS_LOCK_*, GIT_FETCH_COUNTER) are read from the caller's
...
run_sync_guarded() {
  ...
  CS_HOME="$home" CS_ROOT_OVERRIDE="$ROOT" \
    "$ROOT/bin/cs-fleet-sync.sh" "$@" >"$outf" 2>"$errf"
}
...
test_whole_fleet_all_flag() {
  local home out
  home=$(new_home)
  build_pair "$home" all-behind >/dev/null
  advance_origin "$home" all-behind C1
  build_pair "$home" all-current >/dev/null

  # Explicit --all form (used by bin/cs-bootstrap.sh).
  out=$(run_sync "$home" --all)

  assert_contains "$out" "all-behind: synced" "--all form syncs a behind clone"
  assert_contains "$out" "all-current: already current" "--all form reports a current clone"
  pass "--all sweeps every clone under projects/ like the no-argument form"
}

test_bootstrap_relays_recovered_and_stuck() {
  local home fakebin stuck rec out
  home=$(new_home)
  # Hermetic bootstrap: fake herdr/gh/gh-axi so no live server or network is
  # touched; only the FLEET_SYNC relay lines are asserted.
...
SH
  ...
  out=$(PATH="$fakebin:$PATH" CS_HOME="$home" CS_ROOT_OVERRIDE="$ROOT" \
    "$ROOT/bin/cs-bootstrap.sh" 2>/dev/null)
...
test_orphaned_stale_packed_refs_lock_recovers() {
  ...
  set +e
  CS_FLEET_SYNC_PACKED_REFS_LOCK_RETRIES=2 \
  CS_FLEET_SYNC_PACKED_REFS_LOCK_RETRY_WAIT_SECS=0 \
  CS_FLEET_SYNC_PACKED_REFS_LOCK_AGE_SECS=0 \
...
test_transient_packed_refs_lock_self_clears() {
  ...
  CS_FLEET_SYNC_PACKED_REFS_LOCK_RETRIES=3 \
  CS_FLEET_SYNC_PACKED_REFS_LOCK_RETRY_WAIT_SECS=0 \
