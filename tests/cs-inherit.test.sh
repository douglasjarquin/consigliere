...
[ "$(cat "$CAPO/config/backlog-backend")" = manual ] || fail "backlog-backend copied at seed"
...
main_before=$(shasum -a 256 "$MAIN/data/boss-shared.md" | awk '{print $1}')
out=$(cs_inherit_converge "$MAIN" "$CAPO" 2>&1) || fail "drift convergence failed: $out"
assert_contains "$out" "quarantined data/boss-shared.md drift" "drift is reported as CAPO_SYNC"
...
main_after=$(shasum -a 256 "$MAIN/data/boss-shared.md" | awk '{print $1}')
...
rm "$MAIN/config/backlog-backend"
cs_inherit_backlog_backend "$MAIN/config" "$CAPO/config" || fail "backend absence mirror failed"
assert_absent "$CAPO/config/backlog-backend" "backend absence mirrored at seed"
