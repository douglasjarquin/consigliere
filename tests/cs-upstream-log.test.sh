...
# config/host/upstream.conf resolves the checkout, and a missing ledger SHA warns and
...
FIX="$TMP_ROOT/root"
mkdir -p "$FIX/docs" "$FIX/config/host" "$FIX/data"
printf 'last-reviewed: %s\n' "$BASE_SHA" > "$FIX/docs/upstream-review.md"
printf '%s\n' "$FM" > "$FIX/config/host/upstream.conf"
