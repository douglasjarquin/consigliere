#!/usr/bin/env bash
# GitHub workflow lint gate owned by bin/cs-lint-workflows.sh.
#
# A malformed .github/workflows/*.yml, including a self-broken ci.yml, must fail
# in the local/CI lint path before merge.
set -u
# shellcheck source=tests/lib.sh
. "$(dirname "${BASH_SOURCE[0]}")/lib.sh"

LINT_WF="$ROOT/bin/cs-lint-workflows.sh"
LINT="$ROOT/bin/cs-lint.sh"
INSTALLER="$ROOT/bin/cs-install-actionlint.sh"
...
cs_install_stub_tar_actionlint() {
  local fakebin=$1
  cat > "$fakebin/tar" <<'SH'
#!/usr/bin/env bash
while [ "$#" -gt 0 ]; do
...
test_current_workflows_pass() {
  command -v actionlint >/dev/null 2>&1 || {
    pass 'current workflows skipped (actionlint not on PATH)'
    return 0
  }
  local out rc resolved
  resolved=$(actionlint -version | awk 'NR==1 {gsub(/^v/, "", $0); print; exit}')
  [ "$resolved" = "$REQUIRED" ] || {
    pass "current workflows skipped (actionlint $resolved != pin $REQUIRED)"
    return 0
  }
  rc=0
  out=$("$LINT_WF" 2>&1) || rc=$?
...
test_col0_heredoc_fails_with_clear_error() {
  ...
  tmp=$(cs_test_tmproot cs-lint-wf-col0)
...
printf 'ci.yml:10:9: could not parse as YAML [syntax-check]\n' >&2
exit 1
SH
...
test_valid_fixture_passes() {
  local tmp fakebin out rc
  tmp=$(cs_test_tmproot cs-lint-wf-ok)
  fakebin=$(cs_fakebin "$tmp")
  mkdir -p "$tmp/.github/workflows"
...
test_empty_workflows_dir_fails() {
  local tmp fakebin out rc
  tmp=$(cs_test_tmproot cs-lint-wf-empty)
  fakebin=$(cs_fakebin "$tmp")
  mkdir -p "$tmp/.github/workflows"
  cat > "$fakebin/actionlint" <<'SH'
#!/usr/bin/env bash
...
test_explicit_broken_path_fails() {
  local tmp fakebin broken out rc
  tmp=$(cs_test_tmproot cs-lint-wf-path)
  fakebin=$(cs_fakebin "$tmp")
...
#!/usr/bin/env bash
if [ "$1" = "-version" ]; then printf '1.7.12\n'; exit 0; fi
printf 'broken.yml:10:9: could not parse as YAML [syntax-check]\n' >&2
exit 1
SH
  chmod +x "$fakebin/actionlint"
  rc=0
  out=$(PATH="$fakebin:$PATH" "$LINT_WF" "$broken" 2>&1) || rc=$?
...
test_non_mapping_root_fails() {
  local tmp fakebin out rc
  tmp=$(cs_test_tmproot cs-lint-wf-scalar)
  fakebin=$(cs_fakebin "$tmp")
  mkdir -p "$tmp/.github/workflows"
...
#!/usr/bin/env bash
if [ "$1" = "-version" ]; then printf '1.7.12\n'; exit 0; fi
printf 'ci.yml:1:1: mapping node is expected [syntax-check]\n' >&2
exit 1
SH
  chmod +x "$fakebin/actionlint"
  rc=0
...
test_missing_actionlint_fails_closed() {
  ...
  tmp=$(cs_test_tmproot cs-lint-wf-noactionlint)
  fakebin=$(cs_fakebin "$tmp")
  mkdir -p "$tmp/.github/workflows"
  write_valid_workflow "$tmp/.github/workflows/ci.yml"
  ...
  assert_contains "$out" "cs-install-actionlint.sh" \
    "missing actionlint did not name the pinned installer"
  pass "missing actionlint fails closed"
}

test_pins_an_explicit_version() {
  [ -n "$REQUIRED" ] || fail "cs-lint-workflows.sh --required-version printed nothing"
  assert_contains "$REQUIRED" "1.7.12" "cs-lint-workflows.sh must pin actionlint 1.7.12"
  pass "cs-lint-workflows.sh pins an explicit actionlint version ($REQUIRED)"
}

test_rejects_wrong_actionlint_version() {
  local tmp fakebin out rc
  tmp=$(cs_test_tmproot cs-lint-wf-ver)
  fakebin=$(cs_fakebin "$tmp")
  mkdir -p "$tmp/.github/workflows"
  write_valid_workflow "$tmp/.github/workflows/ci.yml"
  cat > "$fakebin/actionlint" <<'SH'
...
SH
  ...
  rc=0
  out=$(PATH="$fakebin:$PATH" "$LINT_WF" --root "$tmp" 2>&1) || rc=$?
  [ "$rc" -ne 0 ] || fail "cs-lint-workflows.sh accepted an actionlint version other than the pin"$'\n'"$out"
  assert_contains "$out" "$REQUIRED" "cs-lint-workflows.sh did not name the required version on mismatch"
  assert_contains "$out" "0.0.0" "cs-lint-workflows.sh did not report the resolved (wrong) version"
  pass "cs-lint-workflows.sh refuses to lint under a non-pinned actionlint version"
}

test_installer_retries_transient_download_failure() {
  local tmp fakebin destination out
  tmp=$(cs_test_tmproot cs-actionlint-download)
  fakebin=$(cs_fakebin "$tmp")
  destination="$tmp/bin"
  ...
  cs_install_stub_tar_actionlint "$fakebin"
...
test_installer_selects_platform_archive_url_and_checksum() {
  ...
  tmp=$(cs_test_tmproot cs-actionlint-platform)
  fakebin=$(cs_fakebin "$tmp")
  destination="$tmp/bin"
...
test_installer_rejects_wrong_checksum() {
  ...
  tmp=$(cs_test_tmproot cs-actionlint-badsum)
  fakebin=$(cs_fakebin "$tmp")
  destination="$tmp/bin"
...
test_installer_falls_back_to_shasum() {
  ...
  tmp=$(cs_test_tmproot cs-actionlint-shasum)
  fakebin=$(cs_fakebin "$tmp")
  destination="$tmp/bin"
...
test_installer_prefers_sha256sum_over_shasum() {
  ...
  tmp=$(cs_test_tmproot cs-actionlint-sha256sum-pref)
  fakebin=$(cs_fakebin "$tmp")
  destination="$tmp/bin"
  hasher_log="$tmp/hasher.log"
...
test_installer_rejects_unsupported_platform() {
  local tmp fakebin destination out rc
  tmp=$(cs_test_tmproot cs-actionlint-unsupported)
  fakebin=$(cs_fakebin "$tmp")
  destination="$tmp/bin"
...
test_cs_lint_default_path_catches_broken_ci_yml() {
  local tmp fakebin log diff_file out rc
  tmp=$(cs_test_tmproot cs-lint-wf-default)
  mkdir -p "$tmp/bin" "$tmp/.github/workflows"
  cp "$LINT" "$tmp/bin/cs-lint.sh"
  cp "$LINT_WF" "$tmp/bin/cs-lint-workflows.sh"
  chmod +x "$tmp/bin/cs-lint.sh" "$tmp/bin/cs-lint-workflows.sh"
  write_col0_heredoc_workflow "$tmp/.github/workflows/ci.yml"
...
case "$*" in
  ...
  "diff --name-only --diff-filter=ACMR -z fakebase123 --")
    [ -n "${CS_TEST_GIT_DIFF_FILE:-}" ] && cat "${CS_TEST_GIT_DIFF_FILE}"
    exit 0
    ;;
  *) exit 0 ;;
esac
...
SH
  ...
  out=$(PATH="$fakebin:$PATH" GITHUB_ACTIONS='' CI='' \
    CS_TEST_GIT_DIFF_FILE="$diff_file" "$tmp/bin/cs-lint.sh" 2>&1) || rc=$?
  [ "$rc" -ne 0 ] || fail "cs-lint.sh default path missed a broken ci.yml"$'\n'"$out"
  assert_contains "$out" "could not parse as YAML" \
    "cs-lint.sh default path did not surface the workflow YAML error"
  assert_contains "$out" "ci.yml" \
    "cs-lint.sh default path did not name the broken workflow"
  pass "cs-lint.sh default path catches a self-broken ci.yml"
}
