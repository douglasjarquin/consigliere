...
#   Usage: cs-promote.sh <task-id>. Reads STATE from CS_STATE_OVERRIDE (or
#   CS_HOME/state) and requires state/<id>.meta to contain the exact line
#   "kind=scout". On success it rewrites the meta removing every kind= line and
#   appending "kind=ship" (all other meta lines preserved, in order), prints
#   "promoted <id> to ship (teardown protection restored)" and a "next:" line
...
# 1. happy path: a scout task flips to ship, other fields preserved.
cs_write_meta "$TMP/state/s1.meta" \
  "project=$TMP/proj-s1" \
  "pane=w1:p1" \
  "kind=scout" \
  "mode=local-only"
out=$("$BIN" s1 2>&1) || fail "scout promotion failed: $out"
...
assert_grep "mode=local-only" "$TMP/state/s1.meta" "the mode field is preserved"
pass "cs-promote flips a scout task's meta to kind=ship, preserving other fields"

# 2. missing meta -> exit 1, names the missing meta.
set +e
out=$("$BIN" ghost 2>&1); rc=$?
set -e
