#!/usr/bin/env bash
# End-to-end tests for durable boss-held decisions discovered by investigations
# and visual reviews (bin/cs-decision-hold.sh plus the cs-teardown.sh gate).
# Requires a real tasks-axi; herdr/gh/gh-axi are faked. The status-transfer verb
# stays literally 'captain-held' (cs-classify-lib.sh) for format compatibility.
set -u
...
TEARDOWN="$ROOT/bin/cs-teardown.sh"
TMP_ROOT=$(cs_test_tmproot cs-decision-hold)
...
SH
  ...
  cs_fake_exit0 "$fakebin" no-mistakes gh gh-axi
  printf '%s\n' "$home"
}

run_teardown() {  # <home> <id>
  local home=$1 id=$2
  PATH="$home/fakebin:$PATH" CS_ROOT_OVERRIDE="$ROOT" CS_HOME="$home" \
    CS_STATE_OVERRIDE="$home/state" CS_DATA_OVERRIDE="$home/data" \
    "$TEARDOWN" "$id"
}
...
run_decisions() {  # <home> <command args...>
  ...
  PATH="$home/fakebin:$PATH" REAL_TASKS_AXI="$TASKS_AXI_BIN" \
    CS_HOME="$home" CS_STATE_OVERRIDE="$home/state" CS_DATA_OVERRIDE="$home/data" \
    "$ROOT/bin/cs-decision-hold.sh" "$@"
}

write_origin_meta() {  # <home> <id> [kind]
  local home=$1 id=$2 kind=${3:-scout}
  cs_write_meta "$home/state/$id.meta" \
...
# open_decisions <home> <id>: the durable status fold from cs-classify-lib.sh.
open_decisions() {
  bash -c '. "$1"; status_open_decisions "$2"' _ \
    "$ROOT/bin/cs-classify-lib.sh" "$1/state/$2.status"
}

# Reproduces the loss exactly with privacy-safe synthetic names: the investigation
# and visual review have ended, the only genuine unresolved decision is report prose,
# and no held backlog item or open status exists. Completion must refuse before
# teardown can erase the source.
...
The boss still needs to choose route north or route south before follow-up work starts.
EOF
...
test_structured_holds_survive_teardown_and_route_resolution() {
  local home id route_hold access_hold open show
...
A separate recommendation is already resolved and requires no boss action.
EOF

  if run_decisions "$home" complete "$id" route access > "$home/early-complete.out" 2> "$home/early-complete.err"; then
    fail "completion succeeded before unresolved decisions had boss holds"
  fi
  ...
  route_hold=$(run_decisions "$home" hold "$id" route \
    --title "Choose the sample route" --reason "boss route choice pending" --repo sample) \
  ...
  run_decisions "$home" hold "$id" route \
    --title "Choose the sample route" --reason "boss route choice pending" --repo sample >/dev/null \
  ...
  assert_grep "captain-held [key=route]" "$home/state/$id.status" \
    "status transfer must keep the literal captain-held verb"
  open=$(open_decisions "$home" "$id")
  ...
  show=$(tasks_in "$home" show "$access_hold" --full)
  assert_contains "$show" "held: yes" "teardown or archival erased the access boss hold"
  ...
  if run_decisions "$home" resolve "$id" route --decision-file "$home/route-decision.txt" \
    ...
    fail "boss hold closed before dependent work had a durable routing edge"
  fi
  show=$(tasks_in "$home" show "$route_hold" --full)
...
if [ "${1:-}" = unblock ] && [ "${2:-}" = sample-route-implementation ] \
  && [ ! -f "$CS_HOME/unblock-failed-once" ]; then
  : > "$CS_HOME/unblock-failed-once"
  exit 1
fi
exec "$REAL_TASKS_AXI" "$@"
EOF
  ...
  if run_decisions "$home" resolve "$id" route --decision-file "$home/changed-route-decision.txt" \
    ...
    fail "partial resolution retry accepted a different boss decision"
  fi
  ...
  if run_decisions "$home" resolve "$id" route --decision-file "$home/changed-route-decision.txt" \
    ...
    fail "resolution retry accepted a different boss decision"
  fi
  if run_decisions "$home" resolve "$id" route --decision-file "$home/route-decision.txt" \
  ...
  assert_contains "$show" "Resolution recorded by cs-decision-hold" "resolved hold lost the decision record"
  show=$(tasks_in "$home" show sample-route-implementation --full)
  assert_contains "$show" "blocked: no" "recorded decision did not release dependent work"
  show=$(tasks_in "$home" show "$access_hold" --full)
  assert_contains "$show" "held: yes" "resolving one decision must not release the other"
  pass "boss holds are idempotent, distinct, teardown-safe, and durably routed before close"
}
...
test_visual_review_uses_shared_completion_owner() {
  ...
  printf '# Sample board investigation\n\nThe initial findings need no boss choice.\n' > "$home/data/$id/report.md"
  ...
  hold=$(run_decisions "$home" hold "$id" layout \
    --title "Choose the sample layout" --reason "boss layout choice pending" --repo sample) \
  ...
  assert_contains "$show" "held: yes" "ended visual review did not leave its durable boss hold"
  assert_contains "$show" "hold_kind: captain" "visual-review hold lost the captain hold kind"
...
The recommendation is informational and needs no boss action.
EOF
  run_decisions "$home" complete "$id" --none >/dev/null \
...
test_terminal_single_owner_status_decision_does_not_block_empty_inventory() {
  ...
  printf 'needs-decision [key=default]: choose route A or route B\ndone: report complete\n' \
    > "$home/state/$id.status"
  printf '# Terminal sample review\n\nNo unresolved boss choice remains.\n' > "$home/data/$id/report.md"
  open=$(open_decisions "$home" "$id")
  ...
  capo=sample-capo
  write_origin_meta "$home" "$capo" capo
  printf 'needs-decision [key=route]: choose route A or route B\ndone: heartbeat complete\n' \
    > "$home/state/$capo.status"
  if run_decisions "$home" complete "$capo" --none \
    > "$home/capo-terminal.out" 2> "$home/capo-terminal.err"; then
    fail "capo terminal status decision was incorrectly cleared"
  fi
  pass "terminal single-owner stale status decisions do not block empty inventory"
}

test_capo_hold_stays_in_authoritative_home() {
  local parent mate origin hold
  parent=$(make_home main-routing)
  mate=$(make_home sample-mate-home)
  origin=sample-mate-review
  mkdir -p "$mate/data/$origin"
  tasks_in "$mate" add "$origin" "Investigate capo sample" --kind scout --repo sample --start >/dev/null
  write_origin_meta "$mate" "$origin"
  printf 'done: report and visual review complete\n' > "$mate/state/$origin.status"
  printf '# Sample capo review\n\nOne boss choice remains.\n' > "$mate/data/$origin/report.md"
  hold=$(run_decisions "$mate" hold "$origin" release \
    --title "Choose the sample release" --reason "boss release choice pending" --repo sample) \
    || fail "capo-owned hold creation failed"
  run_decisions "$mate" complete "$origin" release >/dev/null \
    || fail "capo-owned completion failed"
