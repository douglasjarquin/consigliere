...
# 3. A capo home (config/activation=always) activates with no .afk at all.
dir=$(make_home capo-always)
printf 'always\n' > "$dir/config/activation"
out=$(run_activate "$dir")
...
: > "$dir/state/.afk"
out=$(run_activate "$dir")
assert_contains "$out" "off:" "config/activation=off must disable activation"
...
pass "an unrecognized config/activation refuses"
