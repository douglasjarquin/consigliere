...
make_home() {
  ...
  mkdir -p "$dir/state" "$dir/config/host" "$fakebin"
...
# 3. A capo home (config/host/activation.conf=always) activates with no .afk at all.
dir=$(make_home capo-always)
printf 'always\n' > "$dir/config/host/activation.conf"
out=$(run_activate "$dir")
...
: > "$dir/state/.afk"
out=$(run_activate "$dir")
assert_contains "$out" "off:" "config/host/activation.conf=off must disable activation"
...
pass "an unrecognized config/host/activation.conf refuses"
