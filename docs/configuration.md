...
- `state/.subsuper-daemon-beat` - the away daemon's proof that it is supervising, written by `bin/cs-daemon.sh` at the BOTTOM of each loop pass, so the early-continue paths (pane gone, watcher crash backoff) deliberately do not write it. Contents are a strictly increasing pass counter; mtime is its freshness. `bin/cs-monitor.sh` requires it fresh before standing down. A live pid is not proof: it survives a recycled pid and a daemon wedged off its loop.
