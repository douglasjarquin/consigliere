...
- `state/<id>.meta` - written by `cs-spawn.sh`: `workspace=`, `pane=`, `worktree=`, `project=`, `kind=` (ship|scout|capo), `container=` (ship/scout only: `workspace` for the ordinary dedicated container, `tab` when nested inside a capo's own live workspace - `docs/herdr.md` "Nested tab shape"), `harness=` (codex|claude|grok|cursor, inherited from the root session), and `herdr_session=` (the task endpoint's Herdr session).
  ...
  A relaunch or capo respawn that rotates `endpoint_generation=` also appends `previous_endpoint_generation=` and `previous_endpoint_generation_at=` (skipped only when there was no prior value) so `bin/cs-meta-lib.sh`'s `cs_meta_endpoint_generation_known` keeps validating a message already queued against the superseded generation; `bin/cs-spawn.sh`'s relaunch branch does this via `cs_meta_set` (append, unbounded history), and its capo-spawn branch (which a dead-capo respawn also runs, since a capo can never take the relaunch branch) does it via `cs_meta_endpoint_generation_rotation_lines`, which must re-emit every pair already on file before a `cs_meta_write` full-overwrite or an earlier generation's history is lost (incident, 2026-09-05: this capo-spawn bookkeeping was entirely missing, permanently orphaning a message from a respawned capo and blocking `bin/cs-teardown.sh` fleet-wide via `bin/cs-recover.sh`'s any-failure exit).
  `home=` is written only for a capo (its own isolated CS home, distinct from `parent_home=`); `bin/cs-meta-lib.sh`'s `cs_meta_home()` is the canonical reader for a task's effective home, falling back to `parent_home=` for every ordinary (non-capo) task (incident, 2026-09-05: `bin/cs-inbox.sh`, `bin/cs-recover.sh`, `bin/cs-turnend-guard.sh`, `bin/cs-worker-turnend.sh`, and `bin/cs-spawn.sh` all compared `home=` literally with no fallback, most consequentially blocking every durable inbox acknowledgement for an ordinary child; fixed by `cs_meta_home()`).
  The separate "root wake was not confirmed" live-pane-confirmation defect in `bin/cs-recover.sh`'s `wake_root_message` is not fixed by that change and may still block `bin/cs-teardown.sh` for a message that reaches it - see PR #178 (`https://github.com/douglasjarquin/consigliere/pull/178`) for that defect's own ownership.
...
- `state/.home-pane` - the pane id of THIS home's own agent, written by `bin/cs-session-start.sh` from `HERDR_PANE_ID`, gated on `cs_primary_scope_matches` so a soldier's own task-worktree session reaching this script directly (AGENTS.md tells every session to run it) can never overwrite it with its own pane id. A durable HINT, never an identity: herdr recycles pane ids, so `bin/cs-activate.sh` revalidates that the pane still exists, still runs an agent, and is still rooted in this home before it will prompt anything.
...
- `state/.checkpoint-turn` - the per-turn checkpoint counter. `bin/cs-watch-checkpoint.sh` writes it on entry and refuses (exit 3) if it is already there; `bin/cs-turnend-guard.sh` clears it at every turn end and `bin/cs-session-start.sh` clears one left by a turn that died mid-flight, gated on `cs_primary_scope_matches` so a soldier's own task-worktree session cannot clear the primary's live marker. `docs/supervision.md` owns why the limit is mechanical rather than written down. Safe to delete by hand, which only permits one extra checkpoint in the current turn.
...
| `CS_SPAWN_LAUNCH_WAIT_SECS` | cs-spawn | seconds an interactive launch may take to reach `interactive_ready`: converted to native `agent start --timeout` milliseconds and CLAMPED, with a stderr note, at herdr's 300000ms ceiling when set above 300. Also the default for `CS_CONTROL_RESUME_WAIT_SECS`; default 60 |
...
| `CS_SPAWN_ENV_STEP_WAIT_SECS` | cs-spawn | seconds to wait for the pre-agent-start environment export (`CLAUDE_CONFIG_DIR` under a work/personal account split; a capo's `CS_HOME` and override clears) to be confirmed executed on the pane's shell before the agent starts. Short on purpose: the export is near-instant once the shell is ready to read it; default 10 |
| `CS_SPAWN_BRIEF_DELIVER_WAIT_SECS` | cs-spawn | elapsed-wall-time window for retrying the follow-up brief delivery after a bare `agent start`: a prompt sent within ~40s of agent start can be silently lost while herdr still reports `interactive_ready`, so the window is sized to retry past that. On expiry the spawn warns loudly and still reports spawned (the agent and worktree are kept; steer the brief in manually); default 50 |
| `CS_SPAWN_BRIEF_DELIVER_RETRY_SECS` | cs-spawn | seconds between brief-delivery attempts inside that window; default 5 |
