...
| `CS_SESSION_START_ACTIVE_LIMIT` | cs-session-start | in-flight, held, blocked, and public-followup backlog rows per group in the session-start digest; default 40; each row is shown in full and any remainder is disclosed with the command that prints the rest |
