...
- `data/memory-archive.md` - the cold tier of this home's curated memory: entries `skills/vault` retired from `config/boss.md`, `config/boss-shared.md`, or `config/learnings.md` because they went stale, were proved obsolete, or were evicted over budget with the boss's approval. Never read at session start and never injected into a session, so it costs nothing until a sweep deliberately opens it. Each entry keeps its provenance (original file, date, reason) in a trailing HTML comment. `skills/vault` owns the tier markers, the decay clocks, and what may move here; `bin/cs-vault-cascade.sh` reports the file's size per home and never writes it.
