...
.cursor/             Cursor Agent CLI hooks.json and Cloud Agent environment.json, committed; docs/dev-environment.md owns the Cloud Agent contract
...
docker/dev/Dockerfile  dev-tools suite's single Mac/CI/Cloud-Agent container image; docs/dev-environment.md
...
- `state/<id>.meta` - written by `cs-spawn.sh`: `workspace=`, `pane=`, `worktree=`, `project=`, `kind=` (ship|scout|capo), `container=` (ship/scout only: `workspace` for the ordinary dedicated container, `tab` when nested inside a capo's own live workspace - `docs/herdr.md` "Nested tab shape"), `harness=` (codex|claude|grok|cursor, inherited from the root session), and `herdr_session=` (the task endpoint's Herdr session).
  ...
  `cs-spawn.sh` also records `issue=` for board-driven work, `backlog_item=` when `--backlog-item` named the backlog work item this dispatch began (`cs-teardown.sh` records that item done on successful cleanup), and `headless=1` for a headless scout (`codex exec` / `claude -p`); `cs-pr-check.sh` appends `pr=` and any available `pr_head=`.
