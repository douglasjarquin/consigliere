...
| `CS_SESSION_START_QUEUED_LIMIT` | cs-session-start | plain queued backlog rows in the session-start digest; default 20; in-flight, held, and blocked rows are never bounded and done rows are never listed |
