...
A Gate represents one validation run of one gate type (for example, review, test, lint, security) against one exact input SHA and one exact policy hash.
...
- `pending` - a Gate has been created for a given input SHA and policy hash, but the managed validation command has not yet been started.
...
- `passed` - the managed validation command exited with a `passed` outcome for this exact input SHA and policy hash.
...
- `invalidated` - a Gate that had reached `passed` is no longer valid because the Mission's checkpoint SHA advanced past the SHA this Gate validated.

## Transition table

| From | To | Trigger | Guard | Side effects |
|---|---|---|---|---|
| (none) | `pending` | MissionCoordinator determines a Gate is required for the current checkpoint SHA per the Mission's validation_policy | no existing non-invalidated Gate already covers this exact (mission_id, gate_type, input_sha, policy_hash) tuple | Gate row inserted; `gate.created` event |
| `pending` | `running` | managed validation command is launched with `--run-id`, `--workspace`, `--input-sha`, `--policy-hash`, and the current Decision set | runner slot available per GlobalScheduler | `managed_run_id` recorded; `gate.started` event |
| `running` | `passed` | managed command exits `passed` | exit input SHA matches the Gate's `input_sha` exactly | `output_sha` recorded if applicable; `gate.passed` event; Mission phase may advance if this was the last required Gate |
...
| `needs_decision` | `pending` | the Question is answered and a Decision is recorded | Decision's `input_sha` matches this Gate's `input_sha` (or the Decision is `sha_bound` to exactly this SHA); if the SHA has changed, this transition is refused and a fresh Gate is required instead | a new managed validation invocation is started against the same SHA and policy hash, now supplying the Decision set; `gate.rerun_after_decision` event |
...
| `passed` | `invalidated` | Mission's `current_checkpoint_sha` advances past this Gate's `input_sha` | new checkpoint recorded on the Mission | `gate.invalidated` event; Mission returns to needing a fresh Gate for the new SHA before it can reach `ready_for_review` again |

## Required behavior flows (Section 20)

**Passing validation:** `pending -> running -> passed`. Straightforward; no human wait, no repair.

**Human decision:** `pending -> running -> needs_decision`, Made (or the validator) has already exited by the time this state is recorded, a Question and Mission blocker exist, and the flow only continues via `needs_decision -> pending` once a Decision is recorded and a fresh managed invocation is started against the *same* SHA and policy hash. If the SHA has changed in the meantime, this Gate's `needs_decision` state is left as historical record and a new Gate is created instead; the old Decision, if `sha_bound`, does not apply to the new SHA.
...
- **Invariant 18** (validation is tied to input SHA and policy hash): every transition's guard checks the SHA (and implicitly the policy hash, part of the Gate's identity tuple) matches exactly; `needs_decision -> pending` explicitly refuses to proceed if the SHA has drifted.
