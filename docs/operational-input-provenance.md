...
>
> **SEC-01 closed, by removal, not by §7 item 1 alone (2026-08-13).** Item 1 of the migration checklist (§7) did land in production before this note: `bin/cs-daemon.sh` wrapped its distilled soldier text in a neutralized, explicitly-quoted data region before typing it into the `away-supervisor` envelope, closing the laundering sink this whole document traces. `bin/cs-daemon.sh` itself is now deleted outright, retired onto the existing watch/monitor/activate triangle (`data/stow-synthesis-survey/report.md` item S4) - so the sink is gone, not just mitigated. The only remaining constructor of an `away-supervisor` envelope is `bin/cs-activate.sh`, and its message is a fixed, consigliere-authored string that never quotes any soldier-derived text, so the SEC-01 threat model (§1, §3) no longer applies to any live code path. Every `bin/cs-daemon.sh:<line>` citation below is preserved as an accurate historical trace of the vulnerability and its production fix, not a pointer to a file that still exists; §7's remaining items (2 onward) and SEC-02 (§2c item 2, still structural) are unaffected by this closure and remain open.
