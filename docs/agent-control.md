...
- **The harness is not switchable.** A soldier inherits the root session's harness (`AGENTS.md` section 4), so moving one soldier alone would break that inheritance. There is nothing else about the profile to switch: the harness selects the model and the reasoning level itself.
