...
- A launch failure **after** the stop reports plainly that no agent is running and where the work is preserved (worktree, commit, uncommitted count), and marks the journal `failed:launching`. The recorded profile is left describing what the task was dispatched on, because nothing new is running to describe.
