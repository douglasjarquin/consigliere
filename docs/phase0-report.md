...
| Daemon-bound runner spike (C) designed | Done, and executed; verification gate in progress, not yet closed (see `docs/spikes/spike-c-results.md`) |
| Packaging spike (E) designed | Done (design only; not yet executed) |
| Elixir vs. Go recommendation made, with revisit trigger | Done |
| **Spikes actually executed and passing** | **Spikes A and B done and passing (see `docs/spikes/spike-a-results.md`, `docs/spikes/spike-b-results.md`); Spike C executed, three verification-gate rounds have found and fixed thirteen real defects so far, gate not yet closed (see `docs/spikes/spike-c-results.md`); D, E are the next unit of work** |
...
**Update**: Spike C (daemon-bound runner) has been executed; **its verification gate is not yet closed.** A real external `cs-runner` (Go) OS process survived a live `kill -9` of the entire Elixir daemon, detected the death itself via control-channel EOF with zero daemon-side cooperation, terminated the harness process group, and left a manifest, all captured live in a tmux session, with a reconciler classification function reading that manifest afterward. Round 1 of the `omo:reviewer` verification gate reproduced this live and confirmed the headline claim, then rejected the submission with seven real, reproduced blocking defects (a natural harness exit that verified nothing before declaring victory; `EPERM` misread as "confirmed gone," which also unmasked a real macOS zombie-reaping behavior; an oversized control-channel message misread as the daemon dying; two post-spawn failure paths that abandoned an already-running harness; the `dead_unverified` path never actually being reachable; and no guard against degenerate process-group IDs). Round 2 confirmed all seven held under further adversarial testing, then found four more (two more post-spawn abandonment paths, one worse than anything round 1 fixed since it left no manifest recording the orphan's pgid at all; round 1's own fix for the oversized-message defect had traded it for unbounded memory growth; a signal-killed harness reported no differently from a normal exit; and two of three termination call sites silently discarded a final-manifest write failure) plus three documentation defects. Round 3 confirmed all eleven held, then found two more: round 2's own fix for the memory-growth defect had, in closing it, permanently disabled the control channel after any single too-long line -- silently defeating the daemon-death-detection mechanism this spike exists to prove -- and a fourth post-spawn abandonment path (the one manifest-write call site round 2 left unguarded), plus three further documentation defects. All thirteen findings across three rounds are fixed with new RED/GREEN tests; resubmitted a fourth time, outcome pending. Two genuine race conditions found during the original build (a `setsid()` startup TOCTOU, and a shell `trap`-installation timing race) remain fixed and unaffected by any round. See `docs/spikes/spike-c-results.md` for the full account, including this spike's explicit scope narrowing (no real daemon boot/restart sequence or Mission/Attempt/Workspace schema exists yet; only the classification logic itself is proven here) and the protocol responsibilities not yet implemented. Spikes D and E remain not started.
