...
That initial-event behavior is not a new capability for this supervisor: `cs_watch_wait_transition` already runs a level reconcile on every reconnect, reading each pane's status and routing it through `cs_transition_apply`, which clears the per-pane escalation dedupe marker on `working` before the stream is drained at all.
The initial event delivers that same clear a second time through the stream, so the two mechanisms agree rather than one replacing the other, and the level reconcile stays the path that works when the socket delivers nothing.
