...
Native herdr `blocked` surfaces immediately - sub-second via herdr's own server-side plugin event hook writing this home's event spool (`bin/cs-herdr-event-plugin.sh`, `pane.agent_status_changed`) and on the next poll without it; the poll loop is the permanent fail-closed backstop.
